...
- Add a bounded, fail-closed Windows PnP singleton fallback for transient
  libusb-1/UsbDk Apple-device re-enumeration when the exact iPhone serial is
  still present in PnP but `iSerialNumber` is temporarily unreadable.
- Add `ProductType`-aware procedural iPhone/iPad hardware shells to detached
  previews, including Home-button, notch and Dynamic Island families.
- Keep detached-window resizing and reverse-control coordinates aligned to the
  inner display rectangle while the new shell is visible.
- Preserve the existing USB/AirPlay media path and Bluetooth reverse control;
  no physical-device screen-off/lock-screen behavior is included yet.
