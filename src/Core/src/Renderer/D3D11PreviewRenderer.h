...
public:
    ...
    // Adds a visual device body around detached previews without changing the
    // captured frame. Insets/cutout dimensions are normalized to the mirrored
    // display short edge and are applied entirely on the GPU.
    void set_device_frame_profile(bool enabled, float side_inset, float top_inset,
        float bottom_inset, float outer_corner_radius, std::int32_t cutout_kind,
        float cutout_width, float cutout_height, float cutout_top) noexcept;
