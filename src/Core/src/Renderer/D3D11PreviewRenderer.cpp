...
float cornerCoverage(float2 pixel) {
    ...
    float2 localPixel = pixel - shapeOrigin;
    float2 halfSize = outputSize * 0.5;
    float2 corner = max(abs(localPixel - halfSize) - (halfSize - radius), 0.0) / radius;
...
float roundedRectCoverage(float2 pixel, float4 rect, float radius) {
    float2 halfSize = rect.zw * 0.5;
    float2 center = rect.xy + halfSize;
    float safeRadius = max(min(radius, min(halfSize.x, halfSize.y)), 1.0);
    float2 q = abs(pixel - center) - (halfSize - safeRadius);
    float outside = length(max(q, 0.0));
    float inside = min(max(q.x, q.y), 0.0);
    float distancePixels = outside + inside - safeRadius;
    float antialiasWidth = max(fwidth(distancePixels), 0.75);
    return 1.0 - smoothstep(-antialiasWidth, antialiasWidth, distancePixels);
}

float4 frameMain(float4 position : SV_POSITION, float2 uv : TEXCOORD0) : SV_TARGET {
    if (frameEnabled < 0.5) return float4(0.0, 0.0, 0.0, 0.0);
    float alpha = roundedRectCoverage(position.xy,
        float4(0.0, 0.0, outputSize.x, outputSize.y), frameOuterRadius);
    float2 normalized = saturate(position.xy / max(outputSize, float2(1.0, 1.0)));
    float edgeLight = 0.018 * (1.0 - abs(normalized.x * 2.0 - 1.0));
    float3 body = float3(0.045 + edgeLight, 0.046 + edgeLight, 0.050 + edgeLight);
    // Keep the physical display black before the first decoded frame arrives.
    // The real video pass is drawn immediately afterwards into this same
    // rounded inner rectangle, leaving only the graphite hardware bezel visible.
    float screenCoverage = roundedRectCoverage(position.xy, screenRect,
        max(cornerRadius, min(screenRect.z, screenRect.w) * 0.035));
    body = lerp(body, float3(0.0, 0.0, 0.0), screenCoverage);
    return float4(body * alpha, alpha);
}

float4 cutoutMain(float4 position : SV_POSITION, float2 uv : TEXCOORD0) : SV_TARGET {
    int kind = (int)round(cutoutParams.x);
    if (frameEnabled < 0.5 || kind == 0) return float4(0.0, 0.0, 0.0, 0.0);
    float longSize = max(cutoutParams.y, 1.0);
    float shortSize = max(cutoutParams.z, 1.0);
    bool landscape = screenRect.z > screenRect.w;
    float4 cutoutRect;
    if (!landscape) {
        float top = screenRect.y + cutoutParams.w;
        float left = screenRect.x + (screenRect.z - longSize) * 0.5;
        cutoutRect = float4(left, top, longSize, shortSize);
    } else {
        // With no orientation-side metadata available, place the rotated
        // hardware cutout on the leading (left) edge. Keeping it rotated is
        // visually closer to the physical device than leaving a top cutout on
        // a landscape shell; reverse-control geometry is unaffected.
        float left = screenRect.x + cutoutParams.w;
        float top = screenRect.y + (screenRect.w - longSize) * 0.5;
        cutoutRect = float4(left, top, shortSize, longSize);
    }
    float radius = kind == 2 ? shortSize * 0.5 : shortSize * 0.36;
    float alpha = roundedRectCoverage(position.xy, cutoutRect, radius);
...
struct D3D11PreviewRenderer::Impl {
    struct alignas(16) ShapeConstantData {
        ...
        float gamma{1.0F};
        float shape_origin_x{};
        float shape_origin_y{};
        float frame_enabled{};
        float frame_outer_radius{};
        float screen_x{};
        float screen_y{};
        float screen_width{};
        float screen_height{};
    ...
    struct DeviceFrameProfileData {
        bool enabled{};
        float side_inset{};
        float top_inset{};
        float bottom_inset{};
        float outer_corner_radius{};
        std::int32_t cutout_kind{};
        float cutout_width{};
        float cutout_height{};
        float cutout_top{};
    };
    ...
    ComPtr<ID3D11PixelShader> frame_pixel_shader;
    ComPtr<ID3D11PixelShader> cutout_pixel_shader;
    ComPtr<ID3D11BlendState> premultiplied_blend_state;
    ...
    std::mutex device_frame_mutex;
    DeviceFrameProfileData device_frame_profile;
    ...
    void initialize() {
        ...
        const auto frame_pixel_blob = compile_shader(PreviewPixelShaders, "frameMain", "ps_5_0");
        const auto cutout_pixel_blob = compile_shader(PreviewPixelShaders, "cutoutMain", "ps_5_0");
        ...
        check(device->CreatePixelShader(frame_pixel_blob->GetBufferPointer(),
            frame_pixel_blob->GetBufferSize(), nullptr, &frame_pixel_shader),
            "Create frame PixelShader");
        check(device->CreatePixelShader(cutout_pixel_blob->GetBufferPointer(),
            cutout_pixel_blob->GetBufferSize(), nullptr, &cutout_pixel_shader),
            "Create cutout PixelShader");
        ...
        check(device->CreateBlendState(&blend_description, &premultiplied_blend_state),
            "Create premultiplied blend state");
    ...
    void release_device_resources() noexcept {
        ...
        premultiplied_blend_state.Reset();
        cutout_pixel_shader.Reset();
        frame_pixel_shader.Reset();
    ...
    DeviceFrameProfileData device_frame_snapshot() {
        std::scoped_lock lock(device_frame_mutex);
        return device_frame_profile;
    }

    bool device_frame_enabled(const DeviceFrameProfileData& profile) const noexcept {
        return composition_mode && profile.enabled && !IsZoomed(window) &&
            GetPropW(window, L"iPhoneMirrorFullScreen") == nullptr;
    }

    FloatRect device_screen_rect(const DeviceFrameProfileData& profile) const noexcept {
        if (!device_frame_enabled(profile))
            return {0.0F, 0.0F, static_cast<float>(target_width),
                static_cast<float>(target_height)};
        const auto side_ratio = std::clamp(profile.side_inset, 0.0F, 0.30F);
        const auto top_ratio = std::clamp(profile.top_inset, 0.0F, 0.35F);
        const auto bottom_ratio = std::clamp(profile.bottom_inset, 0.0F, 0.35F);
        // Profiles are measured from the display short edge. Recover that
        // display dimension from the expanded client instead of applying the
        // ratios to the outer short edge a second time. This mirrors the WPF
        // hit-test geometry and keeps video, shell and reverse control aligned.
        const auto client_width = static_cast<float>(target_width);
        const auto client_height = static_cast<float>(target_height);
        const auto display_short_edge = client_width <= client_height
            ? client_width / std::max(0.01F, 1.0F + side_ratio * 2.0F)
            : client_height / std::max(0.01F, 1.0F + top_ratio + bottom_ratio);
        const auto side = side_ratio * display_short_edge;
        const auto top = top_ratio * display_short_edge;
        const auto bottom = bottom_ratio * display_short_edge;
        return {side, top,
            std::max(1.0F, client_width - side * 2.0F),
            std::max(1.0F, client_height - top - bottom)};
    }

    void update_shape_constants(UINT width, UINT height, bool enabled, int rotation = 0,
        const media::DecodedFrame* frame = nullptr, float origin_x = 0.0F,
        float origin_y = 0.0F, const DeviceFrameProfileData* frame_profile_override = nullptr) {
        ShapeConstantData values{};
        values.output_width = static_cast<float>(width);
        values.output_height = static_cast<float>(height);
        values.shape_origin_x = origin_x;
        values.shape_origin_y = origin_y;
        ...
        const auto local_frame_profile = frame_profile_override
            ? *frame_profile_override : device_frame_snapshot();
        const auto framed = device_frame_enabled(local_frame_profile);
        const auto screen = device_screen_rect(local_frame_profile);
        const auto display_short_edge = std::min(screen.width, screen.height);
        values.frame_enabled = framed ? 1.0F : 0.0F;
        values.frame_outer_radius = display_short_edge *
            std::clamp(local_frame_profile.outer_corner_radius, 0.0F, 0.50F);
        values.screen_x = screen.x;
        values.screen_y = screen.y;
        values.screen_width = screen.width;
        values.screen_height = screen.height;
        values.cutout_kind = framed ? static_cast<float>(local_frame_profile.cutout_kind) : 0.0F;
        values.cutout_width = display_short_edge *
            std::clamp(local_frame_profile.cutout_width, 0.0F, 0.90F);
        values.cutout_height = display_short_edge *
            std::clamp(local_frame_profile.cutout_height, 0.0F, 0.40F);
        values.cutout_top = display_short_edge *
            std::clamp(local_frame_profile.cutout_top, 0.0F, 0.30F);
        {
    ...
    void draw_device_frame(const DeviceFrameProfileData& profile) {
        if (!device_frame_enabled(profile)) return;
        ...
        context->PSSetShader(frame_pixel_shader.Get(), nullptr, 0);
        update_shape_constants(target_width, target_height, false, 0, nullptr,
            0.0F, 0.0F, &profile);
        context->Draw(3, 0);
    }

    void draw_device_cutout(const DeviceFrameProfileData& profile) {
        if (!device_frame_enabled(profile) || profile.cutout_kind == 0) return;
        D3D11_VIEWPORT viewport{};
        viewport.Width = static_cast<float>(target_width);
        viewport.Height = static_cast<float>(target_height);
        viewport.MinDepth = 0;
        ...
        context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        context->VSSetShader(vertex_shader.Get(), nullptr, 0);
        context->PSSetShader(cutout_pixel_shader.Get(), nullptr, 0);
        update_shape_constants(target_width, target_height, false, 0, nullptr,
            0.0F, 0.0F, &profile);
        constexpr float blend_factor[] = {0, 0, 0, 0};
        context->OMSetBlendState(premultiplied_blend_state.Get(), blend_factor, 0xffffffffU);
        context->Draw(3, 0);
    ...
    void present_black() {
        ...
        if (composition_mode) {
            const auto frame_profile = device_frame_snapshot();
            if (device_frame_enabled(frame_profile)) draw_device_frame(frame_profile);
            else draw_black_background(rounded_window_enabled());
        }
    ...
    void render(const media::DecodedFrame& frame) {
        ...
        const auto frame_profile = device_frame_snapshot();
        const auto framed = device_frame_enabled(frame_profile);
        const auto screen_area = device_screen_rect(frame_profile);
        const float target_aspect = screen_area.width / screen_area.height;
        D3D11_VIEWPORT viewport{};
        viewport.TopLeftX = screen_area.x;
        viewport.TopLeftY = screen_area.y;
        if (target_aspect > source_aspect) {
            viewport.Height = screen_area.height;
            viewport.Width = viewport.Height * source_aspect;
            viewport.TopLeftX += (screen_area.width - viewport.Width) * 0.5F;
        } else {
            viewport.Width = screen_area.width;
            viewport.Height = viewport.Width / source_aspect;
            viewport.TopLeftY += (screen_area.height - viewport.Height) * 0.5F;
        }
        // The native window controller preserves the source/device-frame aspect
        // in integer client pixels. Fill only sub-pixel rounding gaps inside
        // the physical screen rectangle; genuine letterboxing remains intact.
        const float horizontal_gap = screen_area.width - viewport.Width;
        const float vertical_gap = screen_area.height - viewport.Height;
        ...
        const auto draw_nv12 = [&](const D3D11_VIEWPORT& draw_viewport,
            ...
            update_shape_constants(output_width, output_height, apply_corner, rotation, &frame,
                draw_viewport.TopLeftX, draw_viewport.TopLeftY, &frame_profile);
        ...
        if (!use_limited_pass) {
            ...
            context->ClearRenderTargetView(target.Get(), composition_mode ? transparent : black);
            if (composition_mode) {
                if (framed) draw_device_frame(frame_profile);
                else draw_black_background(rounded);
            }
            draw_nv12(viewport,
                std::max<UINT>(1, static_cast<UINT>(std::lround(viewport.Width))),
                std::max<UINT>(1, static_cast<UINT>(std::lround(viewport.Height))),
                rounded, turns);
            if (framed) draw_device_cutout(frame_profile);
        } else {
            ...
            update_shape_constants(
                std::max<UINT>(1, static_cast<UINT>(std::lround(viewport.Width))),
                std::max<UINT>(1, static_cast<UINT>(std::lround(viewport.Height))),
                rounded, 0, nullptr, viewport.TopLeftX, viewport.TopLeftY, &frame_profile);
...
void D3D11PreviewRenderer::set_device_frame_profile(bool enabled, float side_inset,
    float top_inset, float bottom_inset, float outer_corner_radius,
    std::int32_t cutout_kind, float cutout_width, float cutout_height,
    float cutout_top) noexcept {
    if (!impl_) return;
    Impl::DeviceFrameProfileData profile;
    profile.enabled = enabled;
    profile.side_inset = std::clamp(side_inset, 0.0F, 0.30F);
    profile.top_inset = std::clamp(top_inset, 0.0F, 0.35F);
    profile.bottom_inset = std::clamp(bottom_inset, 0.0F, 0.35F);
    profile.outer_corner_radius = std::clamp(outer_corner_radius, 0.0F, 0.50F);
    profile.cutout_kind = std::clamp(cutout_kind, 0, 2);
    profile.cutout_width = std::clamp(cutout_width, 0.0F, 0.90F);
    profile.cutout_height = std::clamp(cutout_height, 0.0F, 0.40F);
    profile.cutout_top = std::clamp(cutout_top, 0.0F, 0.30F);
    {
        std::scoped_lock lock(impl_->device_frame_mutex);
        impl_->device_frame_profile = profile;
    }
    impl_->refresh_requested.store(true, std::memory_order_release);
}
