...
std::int32_t IM_CALL im_session_set_window_device_frame_profile(
    iPhoneMirror::SessionHandle handle, void* hwnd, std::int32_t enabled,
    float side_inset, float top_inset, float bottom_inset,
    float outer_corner_radius, std::int32_t cutout_kind, float cutout_width,
    float cutout_height, float cutout_top) {
    const auto valid = [](float value, float minimum, float maximum) {
        return std::isfinite(value) && value >= minimum && value <= maximum;
    };
    if (!valid(side_inset, 0.0F, 0.30F) ||
        !valid(top_inset, 0.0F, 0.35F) ||
        !valid(bottom_inset, 0.0F, 0.35F) ||
        !valid(outer_corner_radius, 0.0F, 0.50F) ||
        cutout_kind < 0 || cutout_kind > 2 ||
        !valid(cutout_width, 0.0F, 0.90F) ||
        !valid(cutout_height, 0.0F, 0.40F) ||
        !valid(cutout_top, 0.0F, 0.30F)) {
        return fail(iPhoneMirror::Result::InvalidArgument,
            L"Invalid window device-frame profile");
    }
    auto context = find_multi_session(handle);
    if (!context) return fail(iPhoneMirror::Result::InvalidArgument, L"Unknown session handle");
    std::scoped_lock lock(context->renderers_mutex);
    ...
    if (found == context->renderers.end())
        return fail(iPhoneMirror::Result::InvalidArgument, L"Unknown preview window");
    found->second->set_device_frame_profile(enabled != 0, side_inset, top_inset,
        bottom_inset, outer_corner_radius, cutout_kind, cutout_width,
        cutout_height, cutout_top);
    return static_cast<std::int32_t>(iPhoneMirror::Result::Ok);
}
