...
libusb_device* find_device(const QtUsbContext& context,
    ...
    // libusb/UsbDk can briefly lose iSerialNumber while Apple's composite
    // device is rebuilding. Windows PnP still knows the requested parent. Use
    // that evidence only in a true single-phone environment; with two Apple
    // capture parents attached there is no safe way to correlate an anonymous
    // libusb candidate to the requested serial, so fail closed.
    const auto physical_apple_devices = !selection.index && !identity.serial.empty()
        ? device::discover_physical_apple_usb_devices()
        : std::vector<device::PhysicalAppleDevice>{};
    const bool exact_pnp_parent_present = !selection.index && !identity.serial.empty() &&
        device::is_apple_usb_parent_present(identity.serial);
    if (!selection.index && physical_apple_devices.size() == 1 &&
        exact_pnp_parent_present) {
        std::optional<std::size_t> unique_candidate;
        bool ambiguous{};
        for (std::size_t index{}; index < candidates.size(); ++index) {
            auto& candidate = candidates[index];
            // A readable different serial is authoritative and can never be
            // overridden by the PnP fallback.
            if (!candidate.serial.empty() &&
                !apple_usb_serial_equal(candidate.serial, identity.serial))
                continue;

            populate_descriptor_summary(candidate_devices[index],
                candidate_descriptors[index], candidate);
            candidate.quicktime_endpoints = endpoints_for(
                candidate_devices[index], QuickTimeSubclass);
            candidate.quicktime_configuration =
                candidate.quicktime_endpoints.configuration != 0;

            if (require_quicktime && !candidate.quicktime_configuration)
                continue;
            if (!require_quicktime && identity.original_product_id != 0 &&
                candidate.product_id != identity.original_product_id &&
                !candidate.quicktime_configuration)
                continue;
            if (!device::is_apple_mobile_capture_product_id(candidate.product_id) &&
                !candidate.quicktime_configuration)
                continue;
            ...
            unique_candidate = index;
        }
        if (unique_candidate && !ambiguous) {
            selection.index = *unique_candidate;
            selection.match_kind = AppleUsbMatchKind::PnpSingleton;
            logging::write(std::format(
                "usb_identity backend=libusb1 match=pnp_singleton pid={:04x} topology={} require_quicktime={}",
                candidates[*unique_candidate].product_id,
                candidates[*unique_candidate].topology_id, require_quicktime));
        }
    }
    if (!selection.index) {
        logging::write(logging::Level::Warning, "usb", std::format(
            "usb_identity backend=libusb1 match=none require_quicktime={} pnp_exact={} physical_apple_parents={} candidates={}",
            require_quicktime, exact_pnp_parent_present, physical_apple_devices.size(),
            describe_apple_usb_candidates(candidates, identity)));
        return nullptr;
    }
...
libusb_device* find_device_with_settle(const QtUsbContext& context,
    const AppleUsbIdentity& identity, AppleUsbDevice& info, bool require_quicktime,
    std::chrono::milliseconds timeout) {
    const auto deadline = std::chrono::steady_clock::now() + timeout;
    do {
        if (auto* device = find_device(context, identity, info, require_quicktime))
            return device;
...
bool QtUsbConnection::enable_quicktime_configuration(QtUsbContext& context,
    ...
    AppleUsbDevice info;
    libusb_device* device = find_device_with_settle(context, identity, info, false,
        std::chrono::milliseconds(1500));
    if (!device)
        throw std::runtime_error("Apple USB device not found by serial, physical port, or exact PnP identity");
