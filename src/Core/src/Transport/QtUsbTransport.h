...
enum class AppleUsbMatchKind {
    ...
    Serial,
    Topology,
    // Exact Windows PnP evidence for the requested serial, with exactly one
    // physical Apple capture parent and one safe backend candidate. Used only
    // when iSerialNumber is temporarily unreadable during re-enumeration.
    PnpSingleton,
};
